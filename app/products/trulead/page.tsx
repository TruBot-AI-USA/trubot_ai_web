...
export const metadata = {
  ...
  description: "Automate your prospecting with AI. Find, qualify, and enrich high-quality leads in minutes with TruLead.",
};
    ...
    <>
        ...
        <PageLayout className="flex flex-col items-center gap-12 mx-auto md:flex-row py-12 lg:pt-10 lg:pb-20">
          ...
          <div className="relative flex-1 w-full max-w-lg mt-12 md:mt-0 z-10">
            ...
            <div className="absolute top-8 -right-4 md:-right-12 bg-white p-6 rounded-2xl shadow-xl animate-in fade-in slide-in-from-bottom-6 hidden md:block max-w-[240px] z-20">
                 <p className="text-gray-500 text-xs font-bold uppercase tracking-wide mb-2">AI Insight Score</p>
      ...
      <div className="bg-white space-y-0">
        {problemSolution.map((section, index) => (
          <div key={section.id} className={`py-0 ${index % 2 !== 0 ? 'bg-blue-50/30' : 'bg-white'}`}>
            <PageLayout>
              <div className={`flex flex-col lg:flex-row items-center gap-16 ${section.reversed ? 'lg:flex-row-reverse' : ''}`}>
                
                {/* Text Content */}
                <div className="flex-1 space-y-6">
                  <span className={`inline-block px-3 py-1 text-sm font-semibold rounded-full ${section.id === 'problem' ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>
                    {section.badge}
                  </span>
                  ...
                  <div className="space-y-4 pt-4">
                    {section.features.map((feat, i) => (
                      <div key={i} className="flex gap-4 p-4 rounded-xl hover:bg-white hover:shadow-sm transition-all border border-transparent hover:border-gray-100">
                        <div className={`flex-shrink-0 w-12 h-12 rounded-lg flex items-center justify-center ${section.id === 'problem' ? 'bg-red-50 text-red-500' : 'bg-green-50 text-green-500'}`}>
                          <feat.icon size={24} />
                        </div>
                        <div>
                          <h4 className="font-bold text-gray-900 text-lg">{feat.title}</h4>
                          <p className="text-sm text-gray-600 mt-1 leading-relaxed">{feat.description}</p>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <PageLayout>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {processSteps.steps.map((step, idx) => (
              <div key={idx} className="relative p-8 rounded-2xl border border-gray-100 bg-white shadow-lg hover:shadow-xl transition-all hover:-translate-y-1">
                <div className={`text-6xl font-bold opacity-10 absolute top-4 right-4 ${step.color.split(' ')[0]}`}>
                  {step.number}
                </div>
        ...
        <PageLayout>
          ...
          <div className="grid gap-6 md:grid-cols-2 mt-12">
            {featuresGrid.features.map((item) => (
              ...
              >
                <div className={`flex-shrink-0 w-14 h-14 rounded-xl flex items-center justify-center ${item.color}`}>
      ...
      <div className="bg-[#1034A6] text-white py-5 text-center">
         <PageLayout>
             ...
             <p className="text-lg text-blue-100 max-w-2xl mx-auto mt-10 mb-10">
               TruLead continuously learns from interactions, market signals, and evolving buyer behavior—making it smarter every day.
             </p>
         </PageLayout>
      </div>
      ...
      <div className="bg-white py-0">
        <PageLayout>
           <div className="bg-blue-50 rounded-3xl p-8 md:p-16 text-center max-w-4xl mx-auto relative overflow-hidden">
              ...
              {/* Decorative Quotes */}
              <div className="absolute top-4 left-8 text-blue-200 text-9xl font-serif opacity-50">“</div>
           </div>
        </PageLayout>
      </div>
      ...
      <div className="bg-white pb-0">
        <PageLayout>
            <div className="bg-white border border-gray-100 shadow-2xl rounded-3xl p-8 md:p-16 flex flex-col lg:flex-row items-center gap-12">
               <div className="flex-1 space-y-6">
                  <h2 className="text-3xl md:text-4xl font-bold text-navy">{ctaSection.title}</h2>
